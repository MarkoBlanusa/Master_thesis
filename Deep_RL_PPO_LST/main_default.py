...
def env_creator(env_config):
    return TradingEnvironment(
        ...
        total_episodes=env_config.get("total_episodes", 1001),
...
if __name__ == "__main__":
    ...
    # # Batch the datasets
    # train_X = train_X.map_batches(process_batch, batch_size=1)

    # # Save to parquet file
    # train_X.write_parquet("train_X.parquet")

    # for i in range(len(train_X)):
    #     for j in range(train_X.shape[1]):
    #         for k in train_X[i][j]:
    #             if k < 0 or k > 1:
    #                 print("VALUE OUT OF BOUND : ", k)
    ...
    # train_X_ds.write_parquet("train_dataset.parquet")
    # val_X_ds.write_parquet("val_dataset.parquet")
    # test_X_ds.write_parquet("test_dataset.parquet")
    ...
    #     "grad_clip": tune.quniform(0.4, 0.8, 0.05),
    #     "lambda_": tune.quniform(0.97, 0.99, 0.01),
    #     "entropy_coeff": tune.quniform(0.01, 0.1, 0.01),
    ...
    config.resources(num_gpus=1, num_cpus_per_worker=13)
    config.rollouts(
        num_rollout_workers=1,
        rollout_fragment_length=168,  # 4 weeks of data
    ...
    test_env = gymnasium.make("trading_env_default-v0", input_length=40)
    observation_space = test_env.observation_space
    action_space = test_env.action_space

    # Define the paths
    base_dir = r"C:\Users\marko\ray_results\cond_info_seq40_1h_ep1week\PPO_trading_env_ray-v0_f2f9b_00000_0_2024-12-07_00-36-43\checkpoint_000249"
