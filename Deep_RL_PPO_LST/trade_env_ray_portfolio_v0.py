...
class TradingEnvironment(gym.Env):
    ...
    def __init__(
        ...
        # Load your dataset
        self.data = read_parquet("train_portfolio_dataset_40_1h")
    ...
    def step(self, action):
        ...
        if self.limit_bounds:
            ...
            action = action.reshape(num_assets, 2)
            weights = action[:, 0]
            leverages = action[:, 1]
        ...
        if (self.current_step + 1) >= (self.start_idx + self.episode_length) and any(
            ...
            for i in range(num_assets):
                ...
                # Like single-asset logic: desired_position_size = weight * (balance+pos_value)*leverage for that asset
                desired_position_size = (
                    weight_i * (self.balance + self.position_value) * leverage_i
                )
                desired_position_sizes.append(desired_position_size)

                # Check margin tier for allowed leverage
                allowed_leverage, mm_rate, mm_amount, _, _ = self.get_margin_tier(
                    i, abs(desired_position_size)
                )
                allowed_leverages.append(allowed_leverage)
                ...
                difference_size = desired_position_size - effective_current_size
                ...
                desired_unit_size = abs(desired_position_size) / current_price
                ...
                if not (
                    abs(desired_position_size) >= asset_min_size_usdt
                ...
                required_margin = abs(difference_size) / leverage_i
