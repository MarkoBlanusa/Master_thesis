...
def calculate_indicators(df):
    ...
    df["SMA_week"] = df["close"].rolling(window=168).mean()
    df["SMA_month"] = df["close"].rolling(window=672).mean()
    df["SMA_year"] = df["close"].rolling(window=8064).mean()

    df["SMA_week_ethusdt"] = df["ethusdt_close"].rolling(window=168).mean()
    df["SMA_month_ethusdt"] = df["ethusdt_close"].rolling(window=672).mean()
    df["SMA_year_ethusdt"] = df["ethusdt_close"].rolling(window=8064).mean()

    df["SMA_week_bnbusdt"] = df["bnbusdt_close"].rolling(window=168).mean()
    df["SMA_month_bnbusdt"] = df["bnbusdt_close"].rolling(window=672).mean()
    df["SMA_year_bnbusdt"] = df["bnbusdt_close"].rolling(window=8064).mean()

    df["SMA_week_xrpusdt"] = df["xrpusdt_close"].rolling(window=168).mean()
    df["SMA_month_xrpusdt"] = df["xrpusdt_close"].rolling(window=672).mean()
    df["SMA_year_xrpusdt"] = df["xrpusdt_close"].rolling(window=8064).mean()
...
def env_creator(env_config):
    return TradingEnvironment(
        ...
        input_length=env_config.get("input_length", 60),
        ...
        episode_length=env_config.get("episode_length", 720),
...
if __name__ == "__main__":
    ...
    # final_data = (
    #     data.join(ethusdt_close_df, how="left")
    ...
    # np.save("train_data_40_1h.npy", train_X)
    # np.save("val_data_40_1h.npy", val_X)
    # np.save("test_data_40_1h.npy", test_X)
    ...
    if ray.is_initialized():
        ray.shutdown()
    ray.init(ignore_reinit_error=True, object_store_memory=7 * (1024**3))
    ...
    # train_X_ds.write_parquet("train_dataset_40_1h")
    # val_X_ds.write_parquet("val_dataset_40_1h")
    # test_X_ds.write_parquet("test_dataset_40_1h")
    ...
    # checkpoint_path = r"C:\Users\marko\ray_results\1h_672_compounded_bonus\PPO_trading_env_ray-v0_b7755_00000_0_2024-10-17_04-01-26\checkpoint_000120"
