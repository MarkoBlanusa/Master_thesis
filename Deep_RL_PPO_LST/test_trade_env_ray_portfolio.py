...
class TradingEnvironment(gym.Env):
    ...
    def __init__(
        ...
        # Load your dataset
        self.data = read_parquet("test_portfolio_dataset_200_1d")
        ...
        if self.full_episode:
            self.episode_length = self.data.count() - self.input_length - 1
        else:
    ...
    def reset(self, seed=None, **kwargs):
        ...
        if self.full_episode:
            ...
            self.start_idx = self.input_length + 1
        else:
    ...
    def step(self, action):
        ...
        if (self.current_step + 1) >= (self.start_idx + self.episode_length) and any(
            ...
            for idx in asset_order:
                ...
                print(f"LEVERAGE OF ASSET {i} : ", default_leverage)
                ...
                if difference_size > 0 and current_direction == "short":
                    ...
                    if (closing_size) < abs(effective_current_size):
                        ...
                        print(
                            f"REALIZED PNL OF THE PARTIAL CLOSE : ", self.realized_pnl
                        )
                        ...
                        print(
                            f"BALANCE AFTER CLOSING SHORT OF ASSET {i} : ", self.balance
                        )
                ...
                if (
                    ...
                    if (closing_size) < abs(effective_current_size):
                        ...
                        print(
                            f"BALANCE AFTER CLOSING LONG OF ASSET {i} : ", self.balance
                        )
                        self.realized_pnl += pnl - closing_fee - self.opening_fee
                        self.opening_fee = 0
                        pos["size"] = 0
                ...
                if default_leverage > allowed_leverage:
                    print("LEVERAGE OUTPUTED IS NOT ALLOWED : ", default_leverage)
            ...
            if self.current_episode < self.total_episodes:
                ...
                for metric in self.metrics:
                    if metric not in ["rewards", "returns"]:
                        avg_value = np.mean(self.metrics[metric])
    ...
    def log_episode_metrics(self):
        ...
        portfolio_cum_returns = portfolio_values / self.initial_balance
        portfolio_returns = pd.Series(self.episode_metrics["returns"])
    ...
    def plot_episode_metrics(self):
        ...
        ax1.plot(steps, self.history, color="#1f77b4", linewidth=2.5, alpha=0.9)
        ...
        steps = range(len(self.episode_metrics["returns"]))
        ax2.bar(
            steps,
        ...
        # 3. Asset Allocation Weights (if available)
        if "weights" in self.episode_metrics and self.episode_metrics["weights"]:
            fig3, ax3 = plt.subplots(figsize=(14, 7))
            steps = range(len(self.episode_metrics["weights"]))
            weights_df = pd.DataFrame(self.episode_metrics["weights"], index=steps)
            ...
            ax3.set_title("Portfolio Composition Over Time", fontsize=14, pad=20)
            ax3.legend(
                ncol=3,
                fontsize=9,
                frameon=True,
...
def compute_metrics(
    ...
    metrics["Lowest Point (% of Initial Capital)"] = (
        portfolio_cum_returns.min() / initial_value
    )
